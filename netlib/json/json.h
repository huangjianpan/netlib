...
// json::Json j = json::Json::unmarshal(raw, raw + length, errmsg);
// if (errmsg == nullptr) {
...
class Json {
 ...
 public:
  ...
  static Json unmarshal(const char* begin, const char* end,
                        const char*& errmsg) {
    Json j = build(begin, end);
    if (j.is_error()) {
  ...
  static Json unmarshal(const std::string& raw, const char*& errmsg) {
    return unmarshal(raw.c_str(), raw.c_str() + raw.size(), errmsg);
  }
 ...
 private:
  ...
  static Json build(const char* begin, const char* end);

  static Json build(const char* begin, const char* end, const char*& next);

  // build_object: *begin is '{' and *(next - 1) is '}'
  static Json build_object(const char* begin, const char* end,
                           const char*& next);

  // build bool: *begin is 't' or 'f'
  static Json build_bool(const char* begin, const char* end, const char*& next);

  static Json build_numeric(const char* begin, const char* end,
                            const char*& next);

  static Json build_string(const char* begin, const char* end,
                           const char*& next);

  static Json build_array(const char* begin, const char* end,
                          const char*& next);
 ...
 private:
  ...
  union {
    ...
    void* data_;   // the underlaying data of string, object, array, errmsg
  };
...
inline const char* find(const char* p, const char* end, char t) {
  while (p != end && *p != t) {
    ++p;
  }
  return p;
